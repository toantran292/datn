...
Kiến trúc hệ thống được tổ chức thành các lớp chính như minh họa trong Hình~\ref{fig:system_architecture}. Lớp khách hàng (Client Layer) với Web Browser chạy pm-web (Next.js) giao tiếp với lớp ứng dụng qua REST API và WebSocket. Lớp ứng dụng (Application Layer) bao gồm PM Service (NestJS) được chia thành 3 tầng: Controllers tiếp nhận và xử lý HTTP requests, Services chứa business logic và orchestrate operations, Repositories quản lý truy xuất dữ liệu. Lớp dữ liệu (Data Layer) gồm PostgreSQL cho persistent storage (lưu trữ projects, sprints, issues, statuses, assignees, comments, activities) và Redis cho caching và Pub/Sub real-time updates. Lớp lưu trữ (Storage Layer) sử dụng MinIO/S3 để lưu trữ các tệp đính kèm của issues. Ngoài ra, hệ thống tích hợp với dịch vụ AI bên ngoài (OpenAI, Anthropic) để hỗ trợ các tính năng AI như Sprint Summary và Task Breakdown.

\begin{figure}[H]
\centering
\input{images/system_architecture_tikz.tex}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{images/user_groups.png}
...
\textbf{Phân rã chức năng của Team Member:}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/func_team_member.png}
\caption{Sơ đồ phân rã chức năng của Team Member}
\label{fig:func_team_member}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/func_project_lead.png}
\caption{Sơ đồ phân rã chức năng của Project Lead}
\label{fig:func_project_lead}
\end{figure}

\textbf{Phân rã chức năng của Viewer/Stakeholder:}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/func_viewer.png}
\caption{Sơ đồ phân rã chức năng của Viewer/Stakeholder}
\label{fig:func_viewer}
\end{figure}
