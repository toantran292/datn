...
export class FileStorageClient {
  ...
  async getPresignedGetUrls(ids: string[], expirySeconds = 3600): Promise<PresignedGetUrlResponse[]> {
    if (ids.length === 0) return [];

    const response = await fetch(`${this.baseUrl}/files/presigned-get-urls`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ids, expirySeconds }),
    });

    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new HttpException(
        error.message || 'Failed to get presigned URLs',
        response.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
