...
export class VideoProcessor implements DocumentProcessor {
  ...
  async process(
    ...
    if (!this.openai) {
      ...
      if (!initialized) {
        console.warn('OpenAI not configured. Video processing disabled.');
        return [];
      }
    }
    ...
    if (!ffmpeg) {
      try {
        ...
        console.warn('fluent-ffmpeg not installed. Video processing disabled.');
        return [];
      }
    }
    ...
    try {
      ...
      if (!audioBuffer || audioBuffer.length === 0) {
        console.warn(`No audio extracted from: ${metadata.fileName}`);
        return [];
      }
