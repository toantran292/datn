...
export class SearchRepository {
  ...
  async searchMessages(
    ...
    const qb = this.messageRepo
      ...
      .select([
        ...
        `ts_headline('simple', m.content, plainto_tsquery('simple', :query), 'StartSel=<mark>, StopSel=</mark>, MaxWords=50, MinWords=20') as highlight`,
      ])
      .where('m.org_id = :orgId', { orgId })
      .andWhere('m.deleted_at IS NULL')
      .andWhere(`to_tsvector('simple', m.content) @@ plainto_tsquery('simple', :query)`, { query: sanitizedQuery });
    ...
    const results = await qb
      .orderBy(`ts_rank(to_tsvector('simple', m.content), plainto_tsquery('simple', :query))`, 'DESC')
  ...
  /**
   * Simple search using ILIKE (fallback, less performant)
   */
