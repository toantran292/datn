...
export const BoardView = memo(function BoardView({ projectId, issues, sprint, issueStore }: BoardViewProps) {
...
}> = ({ issue, onDragStart, onDragEnd, onDrop, onDragOver, onDragLeave, dragPosition }) => {
  const [isHovered, setIsHovered] = useState(false);
  const key = issue.sequenceId ? `SCRUM-${issue.sequenceId}` : "Không có mã";
      ...
      <div className="flex items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium text-custom-text-300">{key}</span>
          </div>
